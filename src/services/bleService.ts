...
export interface BLEMessage {
  ...
  _arrivalMs: number;           // Date.now() captured immediately on notification
  _arrivalPerfMs: number;       // performance.now() captured immediately on notification
}
...
export class BLEService {
  ...
  // Reusable TextDecoder instance to avoid GC pressure
  ...
  private handleDataNotification(event: any): void {
    ...
    try {
      ...
      this.packetCount++;
      this.lastDataTimestamp = arrivalMs;
      ...
      const processedData: BLEMessage = {
        ...
        timestamp: new Date(arrivalMs).toISOString(),
        deviceRawTimestamp: rawData.timestamp ?? undefined,
        _arrivalMs: arrivalMs,
        _arrivalPerfMs: parseFloat(arrivalPerf.toFixed(3)),
      };
  ...
  getPacketStats(): { received: number; dropped: number } {
    return { received: this.packetCount, dropped: this.dropCount };
  }
